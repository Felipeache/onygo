...
from .services import get_lat_lon, get_postal_code
...
def edit_profile(request):
    if request.method == "POST":
        try:
            ...
            error = str(e)
            messages.error(request, f"Oops!!! nos lutins ont fait une betise. {error}")
    else:
...
def send_message(request, sender, receiver):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            msg = Message(sender=sender, receiver=receiver, text=request.POST.get("text"))
            msg.save()
...
def show_messages(request, sender_id):
    ...
    sent_msgs = []
    received_msgs = (
        Message.objects.filter(sender=sender, receiver=me)
        .distinct()
        .values("sent", "text", "sender")
        .order_by("sent")
    )
    sent_msgs = (
        Message.objects.filter(sender=me, receiver=sender)
        .distinct()
        .values("sent", "text", "sender")
        .order_by("sent")
    )
    ...
    return render(
        ...
        {"msgs_list": msgs_list, "sent_msgs": sent_msgs, "sender": sender},
    )
...
def apply(request, id):
    ...
    if request.method == "GET":
        try:
            ...
            error = str(e)
            if error:
                messages.error(request, "Oops!!! tu as déjà postulé à cet événement")
                return redirect("search_event")
            else:
                messages.error(
                    request,
